const DateLabelCustomTooltips = function(tooltipModel) {
    ...
    const ClassName = {
        ...
        TOOLTIP_BODY_ITEM_VALUE : 'tooltip-body-item-threshold',
    ...
    if (tooltipModel.body) {
        ...
        tooltipBodyItems.forEach((item, i) => {
            ...
            if (item[0].split(':').length > 1) {
                ...
                tooltipBodyItemLabel.style = 'font-size: 80%'
                ...
                tooltipBodyItemValue.style = 'font-size: 80%'
    ...
    tooltip.style.top = `${positionY + tooltipModel.caretY + 15}px`
}

const OpenCloseDateLabelCustomTooltips = function(tooltipModel) {
    // Add unique id if not exist
    const _setCanvasId = () => {
        const _idMaker = () => {
            const _hex = 16
    ...
    if (tooltipModel.body) {
        ...
        tooltipBodyItems.forEach((item, i) => {
            ...
            if (item[0].split(':').length > 1) {
                ...
                const tooltipBodyItemValue = document.createElement(Selector.SPAN)
                tooltipBodyItemValue.className = ClassName.TOOLTIP_BODY_ITEM_VALUE
                const value = item[0].split(': ').pop()
                tooltipBodyItemValue.innerHTML = i === 0 ? (value === '1' ? 'open' : 'close') : value
                tooltipBodyItemValue.style = 'font-size: 80%'
